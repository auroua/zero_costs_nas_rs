...
from detectron2.utils.regnet_utils import REGNET_DICT
...
class BottleneckTransform(nn.Module):
    ...
    def __init__(self, w_in, w_out, stride, norm, activation_class, params, w_mid=False):
        super().__init__()
        w_b = int(round(w_out * params["bot_mul"]))
        w_se = int(round(w_in * params["se_r"])) if params["se_r"] else None
    ...
    def forward(self, x):
        ...
        #     x = layer(x)
        # return x
        x = self.a_af(self.a_bn(self.a(x)))
        if self.se:
            x_mid = self.se(self.b_af(self.b_bn(self.b(x))))
        else:
            x_mid = self.b_af(self.b_bn(self.b(x)))
        x_out = self.c_bn(self.c(x_mid))
        if self.w_mid:
            return x_mid, x_out
        else:
            return x_out
...
class ResBottleneckBlock(CNNBlockBase):
    ...
    def __init__(self, w_in, w_out, stride, norm, activation_class, params, w_mid=False):
        super().__init__(w_in, w_out, stride)
        self.proj, self.bn, self.w_mid = None, None, w_mid
        if (w_in != w_out) or (stride != 1):
            self.proj = conv2d(w_in, w_out, 1, stride=stride)
            self.bn = get_norm(norm, w_out)
        self.f = BottleneckTransform(w_in, w_out, stride, norm, activation_class, params, w_mid=w_mid)
        self.af = activation_class()
...
class AnyStage(nn.Module):
    ...
    def __init__(self, w_in, w_out, stride, d, block_class, norm, activation_class, params, w_mid=False):
        super().__init__()
        self.w_mid = w_mid
        for i in range(d):
            block = block_class(w_in, w_out, stride, norm, activation_class, params, w_mid=w_mid)
    ...
    def __init__(
        ...
        *,
        ...
        for i, (d, w, s, b, g) in enumerate(
            ...
            stage = AnyStage(prev_w, w, s, d, block_class, norm, activation_class, params, w_mid=w_mid)
            name = "s{}".format(i + 1)
...
def build_regnet_backbone(cfg, input_shape):
    model_params = REGNET_DICT[cfg.MODEL.REGNETS.TYPE]
    d, w0, wa, wm, group_w, se_on, se_r = model_params["DEPTH"], model_params["W0"], model_params["WA"], \
                                          model_params["WM"], model_params["GROUP_W"], model_params["SE_ON"], \
                                          model_params["SE_R"]
    return RegNet(
        stem_class=get_stem_fun(cfg.MODEL.REGNETS.STEM_TYPE),
        stem_width=cfg.MODEL.REGNETS.STEM_W,
        block_class=get_block_fun(cfg.MODEL.REGNETS.BLOCK_TYPE),
        depth=d,
        w_a=wa,
        w_0=w0,
        w_m=wm,
        group_width=group_w,
        stride=cfg.MODEL.REGNETS.STRIDE,
        bottleneck_ratio=cfg.MODEL.REGNETS.BOTMUL,
        se_ratio=se_r if se_on else None,
        out_features=cfg.MODEL.REGNETS.OUT_FEATURES,
        w_mid=cfg.MODEL.REGNETS.WITH_MID
    )
