...
class SemanticSegmentor(nn.Module):
    ...
    def forward(self, batched_inputs, T=1.0, inputs_requires_grad=False, all_ones_inputs=False, zero_costs=False):
        """
        Args:
        ...
        if inputs_requires_grad == True:
            images.tensor.requires_grad_(True)
        if all_ones_inputs == True:
            inputs = torch.ones([1] + list(batched_inputs[0]["image"].size())).double().to(images.device)
        ...
        if "sem_seg" in batched_inputs[0]:
            ...
            targets = None
        if inputs_requires_grad or all_ones_inputs:
            outputs = self.sem_seg_head(features, targets, T, zero_costs=True)
            if zero_costs:
                return images.tensor, outputs
            else:
                return outputs
        else:
            results, losses = self.sem_seg_head(features, targets, T)
