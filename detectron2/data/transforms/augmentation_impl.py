...
class NormalizeVal(Augmentation):
    ...
    def get_transform(self, image, sem_seg):
        assert image.shape[-1] == 3, "RandomSaturation only works on RGB images"
        assert sem_seg.ndim == 2, "Segmentation mask only works for two dimensional images"
        return NormalizeTransform(factor=self.factor)
