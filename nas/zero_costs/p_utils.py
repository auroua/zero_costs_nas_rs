...
def get_layer_metric_array(net, metric, mode, metric_array):
    for name, layer in net.named_modules():
        if mode=='channel' and hasattr(layer,'dont_ch_prune'):
            continue
        if isinstance(layer, nn.Conv2d) or isinstance(layer, nn.Linear):
            if name not in metric_array:
                metric_array[name] = torch.sum(metric(layer)).item()
            else:
                metric_array[name] += torch.sum(metric(layer)).item()


def get_layer_metric_snip(net, metric, mode, metric_array):
    for name, layer in net.named_modules():
        if mode=='channel' and hasattr(layer,'dont_ch_prune'):
            continue
        if isinstance(layer, nn.Conv2d) or isinstance(layer, nn.Linear):
...
def get_layer_metric_original(net, metric, mode, metric_array):
    for name, layer in net.named_modules():
        if mode=='channel' and hasattr(layer,'dont_ch_prune'):
            continue
        if isinstance(layer, nn.Conv2d) or isinstance(layer, nn.Linear):
