...
from ..p_utils import get_layer_metric_array, reshape_elements, get_layer_metric_original
...
@measure('fisher_seg', copy_net=False, mode='channel')
def compute_fisher_per_weight(net, batched_inputs, mode, split_data=1):
    device = net.device
    ...
    for sp in range(split_data):
        ...
        net.zero_grad()
        losses = net(batched_inputs[st:en])
    ...
    def fisher(layer):
        if layer.fisher is not None:
            ...
            return torch.zeros(layer.weight.shape[0])  # size=ch
    get_layer_metric_original(net, fisher, mode, metric_array)

    # broadcast channel value here to all parameters in that channel
    # to be compatible with stuff downstream (which expects per-parameter metrics)
    # TODO cleanup on the selectors/apply_prune_mask side (?)
    get_layer_metric_original(net, lambda l: l.weight.shape[1:], mode, metric_array_shape)

    grads_abs = reshape_elements(metric_array, metric_array_shape, device)

    results = 0.0
    for t in grads_abs:
        results += torch.sum(t).item()
    return results