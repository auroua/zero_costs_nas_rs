...
@measure('grasp_seg', copy_net=False, mode='param')
def compute_grasp_per_weight(net, batched_inputs, mode, T=1, num_iters=1, split_data=1):
    # get all applicable weights
    weights = []
    for layer in net.modules():
        if isinstance(layer, nn.Conv2d) or isinstance(layer, nn.Linear):
    ...
    for sp in range(split_data):
        ...
        for _ in range(num_iters):
            # TODO get new data, otherwise num_iters is useless!
            losses = net.forward(batched_inputs[st:en], T)
            losses_val = sum(losses.values())
            grad_w_p = autograd.grad(losses_val, weights, allow_unused=True)
            if grad_w is None:
                grad_w = list(grad_w_p)
            else:
                for idx in range(len(grad_w)):
    ...
    for sp in range(split_data):
        ...
        losses = net.forward(batched_inputs[st:en], T)
        losses_val = sum(losses.values())
        grad_f = autograd.grad(losses_val, weights, create_graph=True, allow_unused=True)
    ...
    metric_array = {}
    get_layer_metric_array(net, grasp, mode, metric_array=metric_array)
