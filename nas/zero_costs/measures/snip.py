...
from ..p_utils import get_layer_metric_snip
...
@measure('snip_seg', copy_net=False, mode='param')
def compute_snip_per_weight_seg(net, batched_inputs, mode, split_data=1):
    for layer in net.modules():
        if isinstance(layer, nn.Conv2d) or isinstance(layer, nn.Linear):
            layer.weight_mask = nn.Parameter(torch.ones_like(layer.weight))
            layer.weight.requires_grad = False
    ...
    metric_array = {}
    get_layer_metric_snip(net, snip, mode, metric_array)
