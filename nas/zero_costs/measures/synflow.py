...
# @measure('synflow_seg', bn=False, mode='param')
@measure('synflow_bn_seg', bn=True, mode='param')
def compute_synflow_per_weight(net, batched_inputs, mode, split_data=1):
    device = net.device
    ...
    net.zero_grad()
    net.double()
    output = net.forward(batched_inputs, all_ones_inputs=True)
    torch.sum(output).backward()
    ...
    def synflow(layer):
        if layer.weight.grad is not None:
            ...
            return torch.zeros_like(layer.weight)
    metric_array = {}
    get_layer_metric_array(net, synflow, mode, metric_array)
